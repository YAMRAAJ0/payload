  ...
  export default function CourseDetailClient({ courseName }: Props) {
    ...
    useEffect(() => {
      const fetchCourse = async () => {
        const res = await fetch(`/api/course?courseName=${courseName}`)
    ...
    <span>{item}</span>
        ...
        <div className="mt-8 p-4">
          ...
          <div className="space-y-2 text-white p-8 bg-gray-900 opacity-80 rounded-lg">
            {course.CourseContent.map((section: { ContentTitle: string | number | bigint | boolean | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<string | number | bigint | boolean | ReactPortal | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | null | undefined> | null | undefined; Content: any[] }, index: Key | null | undefined) => (
              <div key={index as number}>
                <button
                  onClick={() => toggleDropdown(index as number)}
                  ...
                  {activeIndex === index && (
                    ...
                    >
                      <div className="p-4 bg-gray-900 text-gray-100 space-y-2">
                        {section.Content.map((point: { Content: string | number | bigint | boolean | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<string | number | bigint | boolean | ReactPortal | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | null | undefined> | null | undefined }, pointIndex: Key | null | undefined) => (
